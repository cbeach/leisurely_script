...
object Piece_token {
  def apply(owner: Player): Piece_token = owner match {
    case Player_X => Piece_token_X
    case Player_O => Piece_token_O
  }
...
case object Piece_token_X extends Piece_token(Player_X)
case object Piece_token_O extends Piece_token(Player_O)
...
case class TicTacToe() extends GameState("TicTacToe") {
  ...
  val legalMoves: List[(DiscreteCoord2D, Player, GameState) => Boolean] = List(
    // The argument type is specified by the player type in the LegalMove definition
    (coord: DiscreteCoord2D, player: Player, gs: GameState) => {
      // Check condition
      if (gs.graph((coord.x, coord.y)).isEmpty && gs.isCurrentPlayer(player)) {
        // Perform specified action
        player match {
          case Player_X => gs.graph((coord.x, coord.y)).push(Piece_token_X)
          case Player_O => gs.graph((coord.x, coord.y)).push(Piece_token_O)
        }
        gs.nextTurn
  ...
  val endConditions: List[EndCondition] = List(
    ...
    EndCondition(AllPlayers, (gs: GameState) => gs.boardFull() && !gs.threeInARow(CurrentPlayer, Piece_token(gs, CurrentPlayer)), Draw)
  )
  ...
  val inputs: List[ButtonInput[DiscreteCoord2D]] = List(
    ButtonInput(this, DiscreteCoord2D(0, 0), legalMoves), 
    ButtonInput(this, DiscreteCoord2D(0, 1), legalMoves), 
    ButtonInput(this, DiscreteCoord2D(0, 2), legalMoves),
